...
Util.augment(Column, {
    ...
    _drawPoint: function (point, index) {
        ...
        if (_self.get('labels')) {
            ...
            //此处只支持最简单的情况
            if(_self.get('reverse')) {
                point.y = _self.getBaseValue()-point.y;
            }
    ...
    pointToFactorPath: function (point, factor) {
        ...
        if (isInCircle) { //雷达图中显示
            ...
            //是否上下翻转
            var isReverse = _self.get('reverse');
            if (isReverse) {
                ...
                try {
                    var mt = _self.get('canvas').chart.get('plotBack').get('margin')[0];
                } catch (e) {
                    mt = 0;
                }

                //改变偏移量
                var offset = yValue + height * factor - mt;
